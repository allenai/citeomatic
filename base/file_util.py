...
def cache_file(name):
    ...
    if os.system(
        'aws s3 cp "%s" "%s.tmp.%d"' % (name, target_filename, os.getpid())
    ) != 0:
        raise FileNotFoundError('Failed to copy %s' % name)
    assert os.system(
        'mv "%s.tmp.%d" "%s"' % (target_filename, os.getpid(), target_filename)
    ) == 0
...
def iterate_s3_objects(path, max_files=None):
    ...
    if path.startswith('s3://'):
        ...
        if max_files:
            s3_obj_iterator = \
                s3_bucket.objects.filter(Prefix=folder_key).limit(max_files)
        else:
            s3_obj_iterator = s3_bucket.objects.filter(Prefix=folder_key).all()
...
class S3File(object):
    ...
    def flush(self):
        ...
        assert os.system('aws s3 cp "%s" "%s"' % (self._local_name,
                                                  self.name)) == 0
...
def _gzip_file(fileobj, mode, encoding):
    ...
    if 't' in mode or encoding is not None:
        mode = mode.replace('t', '')
        f = _fix_fileobj(gzip.GzipFile(fileobj=fileobj, mode=mode))
...
def open(filename, mode='rb', encoding=None, **kw):
    ...
    if filename.endswith('.gz') and 'no_decompress' not in kw:
        ...
        target = open(
            filename, no_decompress=True, mode=target_mode, encoding=None, **kw
        )
...
def extract_tarfile_from_bytes(b: bytes, dst: str) -> None:
    seekable_f = io.BytesIO(b)
    safe_makedirs(os.path.dirname(dst))
    with tarfile.open(fileobj=seekable_f) as t:
